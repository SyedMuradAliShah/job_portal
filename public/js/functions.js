...
function edit_js_edu(id) {
	...
	$.ajax({
			...
			data: {
				id: $("#ed_edu_id").val(),
				degree_title: $("#ed_degree_title").val(),
				major_subject: $("#ed_major_subject").val(),
				institute: $("#ed_institute").val(),
				edu_country: $("#ed_edu_country").val(),
				edu_city: $("#ed_edu_city").val(),
				completion_year: $("#ed_completion_year").val()
			}
		})
		.done(function (msg) {
			if (msg == 'done') {
...
function add_js_exp() {
	...
	$.ajax({
			...
			data: {
				...
				exp_country: $("#exp_country").val(),
				exp_city: $("#exp_city").val(),
...
function edit_js_exp() {
	...
	$.ajax({
			...
			data: {
				id: $("#ed_exp_id").val(),
				job_title: $("#ed_job_title").val(),
				company_name: $("#ed_company_name").val(),
				exp_country: $("#ed_exp_country").val(),
...
function update_rating() {
	...
	if (returnval) {
		$.ajax({
				type: "POST",
				url: baseUrl + "employer/job_applications/update_rating",
				data: {
					jsid: $("#jsid").val(),
					message: $("#messagerating").val(),
					skillrating: $("input[type=radio][name=skillrating]:checked").val(),
					techrating: $("input[type=radio][name=techrating]:checked").val(),
					commrating: $("input[type=radio][name=commrating]:checked").val(),
					personrating: $("input[type=radio][name=personrating]:checked").val(),
				}
			})
			.done(function (msg) {
				if (msg == 'done') {
					$('#update_rating').modal('toggle');
					$('#emsgrating').html('');
					$("#message").val('');
					$('#msg').html('<div class="alert alert-success"> <a href="#" class="close" data-dismiss="alert">&times;</a> <strong>Success!</strong> Rating has been updated successfully. </div>');
					setInterval(function () {
						location.reload(); 
					}, 700);
				} else {
					$('#msg').html('');
					$('#emsgrating').html('<span class="label label-warning">' + msg + '</span>');
				}
			});
	} else {
		return false;
...
function add_rating() {
	...
	if (returnval) {
		$.ajax({
				type: "POST",
				url: baseUrl + "employer/job_applications/add_rating",
				data: {
					jsid: $("#jsid").val(),
					message: $("#messagerating").val(),
					skillrating: $("input[type=radio][name=skillrating]:checked").val(),
			...
			.done(function (msg) {
				if (msg == 'done') {
					$('#add_rating').modal('toggle');
					$('#emsgrating').html('');
					$("#message").val('');
					$('#msg').html('<div class="alert alert-success"> <a href="#" class="close" data-dismiss="alert">&times;</a> <strong>Success!</strong> Rating has been added successfully. </div>');
					setInterval(function () {
						location.reload(); 
					}, 700);
				} else {
