...
class Job_seekers_model extends CI_Model
{
    ...
    public function save_user_cv()
    {
        ...
        if ($this->get_user_cv($this->session->userdata('user_id'))->num_rows()) {
            $return = $this->db->update('tbl_cv', $data);
            return $return;
        } else {
            $return = $this->db->insert('tbl_cv', $data);
            if ((bool) $return === TRUE) {
    ...
    public function get_default_cv()
    {
        return $this->db->get('tbl_default_cv');
    }

    public function get_user_cv($id)
    {
        $this->db->where('jobseeker_id', $id);
        return $this->db->get('tbl_cv');
    }
